...
storefrontApp.controller('productController', ['$scope', '$window', '$sce', 'catalogService', function ($scope, $window, $sce, catalogService) {
    ...
    function initialize() {
        ...
        catalogService.getProduct(productIds).then(function (response) {
            ...
            var fullDescription = _.find(product.descriptions, function (d) { return d.reviewType === 'FullReview' });
            if (fullDescription) {
    ...
    function selectVariation(variationId) {
        ...
        if (variation) {
            ...
            variation.releaseNotes = _.find(variation.variationProperties, function (p) { return p.name === 'ReleaseNotes' });
